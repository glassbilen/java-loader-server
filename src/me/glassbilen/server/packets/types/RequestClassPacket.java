...
public class RequestClassPacket implements Packet {
	...
	public void onReceive(CNetworkManager manager, String[] args) {
		...
		if (user.getSecuredApp() == null || !user.getSecuredApp().isReady()) {
			...
			user.getOutput().println(TextColor.RED + "User requested a class before choosing a program.");
			user.close();
			return;
		}
		...
		if (found == null) {
			...
			user.getOutput().println(TextColor.RED + "User requested a class but had a app that should be invalid.");
			user.close();
			return;
		}
		...
		try (ByteArrayOutputStream requestedClassStream = new ByteArrayOutputStream()) {
			try (JarFile file = new JarFile(user.getLoadedFile())) {
					...
					if (className.equals(name)) {
						try (InputStream is = file.getInputStream(entry)) {
							while (is.available() > 0) {
								requestedClassStream.write(is.read());
							}
						}
				...
				try {
					user.sendLine("Close", "Unexpected error!",
							"A unexpected error has occurred while serving a class, try restarting the application or try again later.");
				} catch (IOException e) {
					e.printStackTrace();
				}
			...
			try {
				user.sendLine("RequestedClass", className,
						Base64.getEncoder().encodeToString(requestedClassStream.toByteArray()));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			try {
				user.sendLine("Close", "Unexpected error!",
						"A unexpected error has occurred while allocating the class for you, please contact the author or try again later.");
			} catch (IOException e) {
				e.printStackTrace();
			}
